...
export class PhotoEntities1677526485770 implements MigrationInterface {
    name = 'PhotoEntities1677526485770'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`post\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`title\` varchar(120) NOT NULL DEFAULT '', \`content\` text NULL, \`posted\` tinyint NOT NULL DEFAULT 0, \`ownerId\` int NOT NULL, FULLTEXT INDEX \`IDX_e28aa0c4114146bfb1567bfa9a\` (\`title\`), FULLTEXT INDEX \`IDX_d178da0438977d4e6911acaed3\` (\`content\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`photo\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`file_url\` varchar(255) NOT NULL, \`file\` varchar(255) NOT NULL, \`posted\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`category\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`title\` varchar(120) NOT NULL DEFAULT '', \`description\` text NULL, \`ownerId\` int NOT NULL, \`posted\` tinyint NOT NULL DEFAULT 0, FULLTEXT INDEX \`IDX_9f16dbbf263b0af0f03637fa7b\` (\`title\`), FULLTEXT INDEX \`IDX_7b7115fda47b20b277b8ca6f89\` (\`description\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`category_photos_photo\` ADD CONSTRAINT \`FK_1e56ba761f143efadbacd9c3d6b\` FOREIGN KEY (\`photoId\`) REFERENCES \`photo\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
